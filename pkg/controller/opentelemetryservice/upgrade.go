...
func (r *ReconcileOpenTelemetryService) applyUpgrades(ctx context.Context) error {
	otelcol := ctx.Value(opentelemetry.ContextInstance).(*v1alpha1.OpenTelemetryService)
	...
	if len(otelcol.Status.Version) > 0 {
		if otelcol.Status.Version != currentVersions.OpenTelemetryService {
	...
	otelcol.Status.Version = version.Get().OpenTelemetryService
